...
function prettyTag(t){ return t.replace(/-/g,' '); }
...
function selectTab(id){
  if(id.startsWith('coll:')){ S.tab='saved'; S.activeColl=id.slice(5); }
  else S.tab=id;
...
function renderSidebar(){
  ...
  // mobile chips
  const chips=[...n.feed, ...n.topics, ...n.library];
...
function visibleList(tab){
  ...
  if(query){
    ...
    list=list.filter(a=>a.title.toLowerCase().includes(q)||(a.snippet||'').toLowerCase().includes(q)||sourceName(a.src).toLowerCase().includes(q)||(a.tags||[]).some(t=>t.includes(q)));
  }
...
/* Why each article was chosen — maps its topics back to what Christi asked to follow */
const TAG_WHY = {
  claude:'the latest on Claude & Anthropic', agents:'AI agents & agentic workflows',
  automation:'automating things with AI', chatgpt:'making ChatGPT work for you',
  models:'the latest AI models', howto:'AI tutorials & how-tos', tutorials:'AI tutorials & how-tos',
  tools:'the latest AI tools & extensions', startups:'the latest AI startups',
  'ai-money':'making money with AI', 'ai-building':'building websites & apps with AI',
  content:'producing content with AI', plugins:'AI plugins & extensions', skills:'AI skills',
  business:'running a business with AI', 'ai-work':'using AI for your work', research:'AI research',
  prompting:'AI prompting skills', engineering:'how AI systems get built', tech:'the tech landscape',
  'ai-news':'the latest AI news',
  supplements:'supplements', nootropics:'nootropics', wellness:'health & wellness',
  fitness:'exercise & fitness hacks', workouts:'workout ideas', abs:'ab & core workouts',
  'abs-core':'ab & core workouts', herbs:'herbal & at-home remedies', remedies:'herbal & at-home remedies',
  natural:'natural living', holistic:'alternative & holistic medicine', 'functional-med':'functional medicine',
  nutrition:'the benefits of foods & nutrients', evidence:'evidence-based health',
  recipes:'healthy high-protein recipes', 'meal-prep':'easy meal prep', protein:'high-protein eating',
  'wellness-startups':'the latest wellness startups', 'health-tech':'health & wellness tech',
  biohacking:'health optimization', 'gut-health':'gut health', sleep:'better sleep', longevity:'longevity',
  'respectful-parenting':'therapy-oriented, shame-free parenting', 'gentle-parenting':'shame-free parenting',
  emotional:'teaching kids emotional regulation', 'emotional-regulation':'teaching kids emotional regulation',
  'child-anxiety':'raising emotionally steady kids', confidence:'raising confident, mentally strong kids',
  'growth-mindset':'raising confident, mentally strong kids', homeschool:'homeschooling & alternative schools',
  'kids-finance':'teaching kids about money', 'kids-and-ai':'preparing kids for an AI world',
  'kids-skills':'raising skilled kids', 'parenting-news':'alternative parenting', gifted:'homeschooling & gifted learning',
};
...
function whyChosen(a){
  const reasons=[...new Set((a.tags||[]).map(t=>TAG_WHY[t]).filter(Boolean))].slice(0,2);
  const base = reasons.length
    ? `Picked because you asked to follow <b>${reasons.join('</b> and <b>')}</b>`
    : `Picked from ${esc(CAT_WHY[a.cat]||'your feed')}`;
...
function renderMeta(done, total){
  ...
  $('#feedmeta').innerHTML=`<span class="live"${NET_BLOCKED?' style="background:var(--warn,#fbbf24);box-shadow:0 0 8px #fbbf24"':''}></span> ${SOURCES.length-S.disabled.length} sources · ${n} articles · ${upd}${prog}${esc(mode)}`;
}
function wireCards(scope){
  $$(scope+' [data-tag]').forEach(b=>b.onclick=e=>{e.stopPropagation();$('#search').value=b.dataset.tag;query=b.dataset.tag;render();});
...
function render(){
  ...
  if(list.length===0){
    ...
    feed.innerHTML=`<div class="empty" style="grid-column:1/-1"><div class="big">${m[0]}</div><h3>${m[1]}</h3><p>${m[2]}</p></div>`;
  } else {
